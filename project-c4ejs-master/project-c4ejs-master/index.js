...
function done(e) {
  ...
  todoDB.forEach((task) => {
    if (task.username == currentUser && task.name == e.target.id.slice(2)) {
      ...
      document.getElementById(`0-${e.target.id.slice(2)}`).style[
        "text-decoration"
      ] = "line-through";
      document.getElementById(`0-${e.target.id.slice(2)}`).style["color"] =
        "green";
      document.getElementById(`0-${e.target.id.slice(2)}`).style["filter"] =
        "blur(0px)";
      return;
    }
  });
